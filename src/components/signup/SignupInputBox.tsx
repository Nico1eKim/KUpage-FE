  ...
  return (
    <div className="flex flex-col mt-8 text-white w-full max-w-full relative">
      <div className="flex justify-between items-center mb-12">
        ...
        {helperText && <span className="text-13 font-400 text-yellow">{helperText}</span>}
      </div>
      <input
        type="text"
        value={value}
        ...
        className="w-full bg-gray border-2 border-[#868686] border-solid rounded-8 px-16 py-12 text-20 transition"
      />
      {isError && <span className="absolute left-8 top-112 text-yellow">{errorText}</span>}
    </div>
  );
};
